"""
Copied from https://github.com/juliaiwhite/amortized-rsa/blob/master/vision.py
"""
import torch
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, indim, outdim, pool=True, padding=1, dropout=None):
...
class ConvNet(nn.Module):
    def __init__(self, depth, flatten=True, dropout=None):
        super(ConvNet, self).__init__()
        trunk = []
        for i in range(depth):
            indim = 3 if i == 0 else 64
            outdim = 64
            dropout = dropout if i != (depth-1) else None # don't apply on last layer
            B = ConvBlock(
                indim, outdim, pool=(i < 4), dropout=dropout)  #only pooling for fist 4 layers
            trunk.append(B)
...
def Conv4(dropout=None):
    return ConvNet(4, dropout=dropout)
